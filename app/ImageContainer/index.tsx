...
const fetchImages = async (query: string): Promise<UnsplashImage[]> => {
  const { data } = await axios.get(`/api/unsplash?query=${query}`);
        ...
        {Array.isArray(images) &&
          ...
          images.map((image) => (
            <div
              ...
              key={image.id}
              className="mb-4 overflow-hidden rounded-lg  shadow-md transition-transform duration-300 cursor-zoom-out ease-in-out"
            >
              <div
                className="relative"
